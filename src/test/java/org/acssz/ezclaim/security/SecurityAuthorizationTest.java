...
class SecurityAuthorizationTest {
  ...
  void setupStubs() {
    ...
    when(tagService.list())
        .thenReturn(List.of(Tag.builder().id("t1").label("L").color("#fff").build()));
  ...
  void reader_cannot_create_tag_admin_can() throws Exception {
    ...
    // Admin has TAG_WRITE
    when(tagService.create(eq("a"), eq("#fff")))
        .thenReturn(Tag.builder().id("t1").label("a").color("#fff").build());
    mvc.perform(
            post("/api/tags")
  ...
  void claims_patch_requires_write_scope_for_fields() throws Exception {
    ...
    org.springframework.security.access.AccessDeniedException denied =
        new org.springframework.security.access.AccessDeniedException("field update not allowed");
  ...
  void patch_anonymous_requires_password_for_protected_status_change() throws Exception {
    ...
    Claim withdrawn =
        Claim.builder()
            .id("cx")
            .title("t")
            .status(ClaimStatus.WITHDRAW)
            .createdAt(protectedClaim.getCreatedAt())
            .updatedAt(Instant.now())
            .build();
  ...
  static class MockBeans {
    ...
    AuditEventService auditEventService() {
      return Mockito.mock(AuditEventService.class);
    }
  ...
  static class TestSecurity {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
      http.csrf(csrf -> csrf.disable()).authorizeHttpRequests(SecurityRules::apply);
      return http.build();
    }
  }
  ...
  void anonymous_must_provide_password_when_protected() throws Exception {
    ...
    mvc.perform(get("/api/claims/cx").param("password", "bad")).andExpect(status().isForbidden());

    // Correct password → 200
    mvc.perform(get("/api/claims/cx").param("password", "secret")).andExpect(status().isOk());
