...
class AuditEventControllerTest {
    ...
    void list_returns_page_with_content() throws Exception {
        ...
        AuditEvent a2 = AuditEvent.builder().id("a2").entityType("T").entityId("2").action("DELETE").occurredAt(Instant.parse("2025-01-02T00:00:00Z")).build();
        Page<AuditEvent> page = new PageImpl<>(List.of(a1, a2), PageRequest.of(0, 20, Sort.by(Sort.Direction.DESC, "occurredAt")), 2);
        when(service.search(isNull(), isNull(), isNull(), isNull(), isNull(), any(Pageable.class))).thenReturn(page);
    ...
    void list_parses_sort_parameter() throws Exception {
        ...
        when(service.search(isNull(), isNull(), isNull(), isNull(), isNull(), any(Pageable.class))).thenReturn(empty);
    ...
    void list_parses_time_range_parameters() throws Exception {
        Page<AuditEvent> empty = new PageImpl<>(List.of(), PageRequest.of(0, 10), 0);
        when(service.search(isNull(), isNull(), isNull(), any(Instant.class), any(Instant.class), any(Pageable.class)))
                .thenReturn(empty);
        ...
        verify(service).search(isNull(), isNull(), isNull(), fromCap.capture(), toCap.capture(), any(Pageable.class));
